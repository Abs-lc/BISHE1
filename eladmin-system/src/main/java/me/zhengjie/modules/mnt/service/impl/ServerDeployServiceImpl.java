...
public class ServerDeployServiceImpl implements ServerDeployService {
    ...
    public void download(List<ServerDeployDto> queryAll, HttpServletResponse response) throws IOException {
        ...
        for (ServerDeployDto deployDto : queryAll) {
            ...
            map.put("服务名称", deployDto.getName());
            map.put("服务ID", deployDto.getIp());
            map.put("所属系统", deployDto.getPort());
            map.put("报文格式", deployDto.getAccount());
            map.put("服务版本", deployDto.getVersion());
            map.put("服务状态", deployDto.getState());
            map.put("服务描述", deployDto.getDescription());
